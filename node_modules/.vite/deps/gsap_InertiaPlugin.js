...
var gsap$1, _coreInitted$1, _toArray$1, _getUnit$1, _first, _ticker, _time1, _getCache$1, _getGSAP$1 = function _getGSAP$2() {
	return gsap$1 || typeof window !== "undefined" && (gsap$1 = window.gsap);
}, _lookup = {}, _round$1 = function _round$2(value) {
	return Math.round(value * 1e4) / 1e4;
}, _getID = function _getID$1(target) {
	...
	deg: 360,
	rad: Math.PI * 2
}, _initCore$1 = function _initCore$2() {
	gsap$1 = _getGSAP$1();
	if (gsap$1) {
...
var VelocityTracker = /* @__PURE__ */ function() {
	...
	VelocityTracker$1.track = function track(targets, properties, types) {
		...
		while (i--) {
			tracker = _getByTarget(targs[i]) || new VelocityTracker$1(targs[i]);
			j = a.length;
...
var gsap, _coreInitted, _parseEase, _toArray, _power3, _config, _getUnit, PropTween, _getCache, _checkPointRatio, _clamp, _processingVars, _getStyleSaver, _reverting, _getTracker = VelocityTracker.getByTarget, _getGSAP = function _getGSAP$2() {
...
}, _bonusValidated = 1, _isArray = Array.isArray, _emptyFunc = function _emptyFunc$1(p) {
	return p;
}, _bigNum = 1e10, _tinyNum = 1 / _bigNum, _checkPoint = .05, _round = function _round$2(value) {
	return Math.round(value * 1e4) / 1e4;
}, _extend = function _extend$1(obj, defaults, exclude) {
	for (var p in defaults) if (!(p in obj) && p !== exclude) obj[p] = defaults[p];
	return obj;
}, _deepClone = function _deepClone$1(obj) {
	var copy = {}, p, v;
	for (p in obj) copy[p] = _isObject(v = obj[p]) && !_isArray(v) ? _deepClone$1(v) : v;
	return copy;
...
}, _initCore = function _initCore$2() {
	gsap = _getGSAP();
	if (gsap) {
